...
/*--------------------------------------GLRU------------------------------------------------*/
void CACHE::updateGLRU(uint32_t set, uint32_t way) {
	
	if (set == mruSet && way == mruWay) {
		return;
	}
	if (set == lruSet && way == lruWay) {

		lruSet = block[set][way].nextSetNum;
		lruWay = block[set][way].nextWayNum;

		block[set][way].prevSetNum = mruSet;
		block[set][way].prevWayNum = mruWay;

		block[mruSet][mruWay].nextSetNum = set;
		block[mruSet][mruWay].nextWayNum = way;
	...
	block[block[set][way].prevSetNum][block[set][way].prevWayNum].nextSetNum = block[set][way].nextSetNum;
	block[block[set][way].prevSetNum][block[set][way].prevWayNum].nextWayNum = block[set][way].nextWayNum;

	block[block[set][way].nextSetNum][block[set][way].nextWayNum].prevSetNum = block[set][way].prevSetNum;
	block[block[set][way].nextSetNum][block[set][way].nextWayNum].prevWayNum = block[set][way].prevWayNum;
