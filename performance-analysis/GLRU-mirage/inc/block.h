...
/*-----------------------------------GLRU---------------------------------------------------*/
    ...
    Link List implementation: 2*log_2(numWays*numSets) bits per block {theoretically reducable to log_2(numWays*numSets) at the cost of complexity}
    And maintain global MRU (i,j) and global LRU (i,j) 
    Initialisation:
        1] (i,j).prev = (i,j-1) if j != 0 and (i-1,NUM_WAY-1) otherwise
        2] (i,j).next = (i,j+1) if j != NUM_WAY-1 and (i+1,0) otherwise
        3] GLRU = (0,0)
        4] GMRU = (NUM_SET-1,NUM_WAY-1)
    On a hit:
        1] (i,j).prev.next = (i,j).next
        2] (i,j).next.prev = (i,j).prev
        3] GMRU.next = (i,j)
        4] (i,j).prev = GMRU
        5] GMRU = (i,j)
    On an insertion:
