...
uint32_t lruSet;
uint32_t lruWay;
void updateGLRU(uint32_t set, uint32_t way);
/*------------------------------------------------------------------------------------------*/
...
/*----------------------------------------CEASER-S----------------------------------------------*/
    ...
    uint32_t *curr_keys[16],*next_keys[16];//CEASER-s
...
/*
  for (uint32_t i=0; i < NUM_SET; i++) {
    for (uint32_t j=0; j < NUM_WAY; j++) {
      block[i][j].prevSetNum = i;
      block[i][j].prevWayNum = j-1;
      block[i][j].nextSetNum = i;
      block[i][j].nextWayNum = j+1;
      if (j == 0) {
        block[i][j].prevSetNum = i-1;
        block[i][j].prevWayNum = NUM_WAY - 1;
      }
      if (j == NUM_WAY - 1) {
        block[i][j].nextSetNum = i+1;
        block[i][j].nextWayNum = 0;
      }
    }
  }
