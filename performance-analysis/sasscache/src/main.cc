...
void print_roi_stats(uint32_t cpu, CACHE *cache)
{
   ...
   cout<< "  HIT %: " << setw(5) << ((double)cache->roi_hit[cpu][2]*100/cache->roi_access[cpu][2]) << "  MISS %: " << setw(5) << ((double)cache->roi_miss[cpu][2]*100/cache->roi_access[cpu][2]);
    ...
    cout << cache->NAME << " Set Associative Eviction Count : " << cache->sae_count << endl;
...
// ##################################
    ...
    while (run_simulation) {
		...
		*/
        ...
        int count_valid = LLC_SET * LLC_WAY * NUM_SLICES - counter;
		cout << " Total Blocks : " << (LLC_SET * LLC_WAY * NUM_SLICES ) << " Invalid Blocks : " << counter << " Valid Blocks : " << count_valid << endl;
        if (MAYA){
            int counter_invalid = 0, counter_pr0 = 0, counter_pr1 = 0;
            for(int set=0;set< LLC_SET;set++)
            {
                for(int way=0;way<uncore.LLC[0]->extra_tag_ways;way++)
                {
                    for(int slice_num=0;slice_num<NUM_SLICES;slice_num++) {
                        if(uncore.LLC[slice_num]->tag0[set][way].valid == 0)
                            counter_invalid++;
                        else if (uncore.LLC[slice_num]->tag0[set][way].valid == 1) {
                            if(uncore.LLC[slice_num]->tag0[set][way].priority == 0)
                                counter_pr0++;
                            else if(uncore.LLC[slice_num]->tag0[set][way].priority == 1)
                                counter_pr1++;
                            else    
                                assert(0);
                        }
                            ...
                            assert(0);
                        if(uncore.LLC[slice_num]->tag1[set][way].valid == 0)
                            counter_invalid++;
                        else if (uncore.LLC[slice_num]->tag1[set][way].valid == 1) {
                            if(uncore.LLC[slice_num]->tag1[set][way].priority == 0)
                                counter_pr0++;
                            else if(uncore.LLC[slice_num]->tag1[set][way].priority == 1)
                                counter_pr1++;
                            else
                                assert(0);
                        }
            ...
            cout << " Total Tags : "<< (LLC_SET * uncore.LLC[0]->extra_tag_ways * 2 * NUM_SLICES ) << " Threshold : " << uncore.LLC[0]->threshold_pr0 << " Invalid Tags : "<< counter_invalid << " Pr0 Tags : " << counter_pr0 << " Pr1 Tags : " << counter_pr1 << " num_pr0 : " << uncore.LLC[0]->num_pr0 << endl;
            cout << " SAE Count : " << (uncore.LLC[0]->sae_count) << endl;
            //assert(uncore.LLC[0]->num_pr0 == counter_pr0);
            assert(uncore.LLC[0]->sae_count == 0);
        }

        uint64_t counter_interference = 0;
        for(int slice_num=0;slice_num<NUM_SLICES;slice_num++) {
            counter_interference = counter_interference + uncore.LLC[slice_num]->interference_count;
        }
        cout << " Inter-Core Inteference in Data Store : " << counter_interference << endl;
